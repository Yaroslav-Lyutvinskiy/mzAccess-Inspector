...
namespace Inspector
{
    public partial class MetaRepForm : Form {
        ...
        string ProgramCaption = "IsoTrack Inspector - v.1.7.0 ";
        ...
        private class RowComparerNumeric : System.Collections.IComparer {
            ...
            public int Compare(object x, object y) {
                ...
                if (ColumnNumber == 4) {  //compare by mode 
        ...
        private void openText_Click(object sender, EventArgs e) {
            ...
            TargetFileName = OpenTextDialog.FileName;
            LFRep(null, null);
            foreach(TargetNum T in TargetIDs) {
                SameMZ(T);
            }
        ...
        private void MarkUpButton_Click(object sender, EventArgs e) {
            MarkUpButton.Checked = !MarkUpButton.Checked;
            if(MarkUpButton.Checked) {
                this.KeyPress += new System.Windows.Forms.KeyPressEventHandler(this.MarkUp_KeyPress);
            }else {
                this.KeyPress -= new System.Windows.Forms.KeyPressEventHandler(this.MarkUp_KeyPress);
            }
        }

        private void MarkUp_KeyPress(object sender, KeyPressEventArgs e) {
            if(e.KeyChar >='0' && e.KeyChar <='5') {
                int RowIndex = ValueView.SelectedCells[0].RowIndex;
                (ValueView.Rows[RowIndex].Cells[0].Value as TargetNum).Desc = e.KeyChar.ToString();
                ValueView.Rows[RowIndex].Cells[1].Value  = e.KeyChar.ToString();
                ValueView.InvalidateRow(RowIndex);
                ValueView.ClearSelection();
                ValueView.Rows[RowIndex + 1].Selected = true;
                ValueView.CurrentCell = ValueView.Rows[RowIndex + 1].Cells[0];
                e.Handled = true;
            }
